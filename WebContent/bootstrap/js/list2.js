...
function build_user_table(result) {
    ...
    $.each(users,function(index,item){
        ...
        var userIdTd = $("<td></td>").append(item.cid);
        var userNameTd = $("<td></td>").append(item.uname);
        var passwordTd = $("<td></td>").append(item.cnumber);
        var timeTd = $("<td></td>").append(item.ctime);

        var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm user-edit-btn")
                                .append($("<span></span>").addClass("glyphicon glyphicon-pencil"))
                                .append("编辑");
        //将编辑按钮添加自定义属性，值为当前数据的id
        editBtn.attr("user-edit-id",item.cid);
        ...
        delBtn.attr("user-del-id",item.cid);
...
function getUserById(id){
    $.ajax({
        ...
        success:function(result){
            //console.log(result);
            // 在模态框相应位置赋值
            $("#userUpdateModal_userId").val(result.data.user.cid);
            $("#userUpdateModal_userName").val(result.data.user.ctime);
            $("#userUpdateModal_userPwd").val(result.data.user.cnumber);
