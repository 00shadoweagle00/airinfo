...
function build_user_table(result) {
    ...
    $.each(users,function(index,item){
        ...
        var passwordTd = $("<td></td>").append(item.upassword);
...
function getUserById(id){
    $.ajax({
        ...
        success:function(result){
            //console.log(result);
            // 在模态框相应位置赋值
            $("#userUpdateModal_userId").val(result.data.user.uid);
            $("#userUpdateModal_userName").val(result.data.user.uname);
            $("#userUpdateModal_userPwd").val(result.data.user.upassword);
...
function checkUserName(){
    ...
    else if(checkRepeatUserName()){
        changeInfo(userNameId,"has-success","用户名输入正确");
        return true;
    }else{
        changeInfo(userNameId,"has-warning","用户名存在");
        return false;
    }
